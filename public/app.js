...
function getSteps(ans) {
  let steps = [...DIMS, 'vibeElement', 'selfCharacter', 'height', 'build', 'hairStyle', 'smoking'];
  if (ans.smoking === 'Socially') steps.push('smokingFollow');
  steps.push('drinking');
  if (ans.drinking === 'Socially') steps.push('drinkingFollow');
  steps.push('diet', 'goal', 'prefHeight', 'prefBuild', 'prefHairStyle', 'prefCharacter', 'aiFollowup', 'idealPartner');
  return steps;
}
...
function sectionLabel(key) {
  ...
  if (['height','build','hairStyle','selfCharacter'].includes(key)) return 'ABOUT YOU';
...
function renderQuiz() {
  ...
  render(`
    ...
    <div class="q-options q-anim">
      ${def.opts.map((o, i) => `<div class="q-opt" onclick='answerQuiz(${JSON.stringify(key)}, ${SCALE_Q[key] ? i - 2 : JSON.stringify(key === 'vibeElement' ? VIBE_VALUE[i] : o)})'>${esc(o)}</div>`).join('')}
    </div>
  `);
}
...
function genderBar(readyCount, joinedCount, label, cls) {
  const readyPct = Math.min(100, (readyCount / 5) * 100);
  const joinedPct = Math.min(100, (joinedCount / 5) * 100);
  return `
    <div class="squad-bar-row">
      <div class="squad-bar-label"><span>${label}</span><span>${readyCount}/5 ready · ${joinedCount}/5 in</span></div>
...
async function renderLobby() {
  ...
  render(`
    ...
    ${banner}
    <div class="card">
      ${genderBar(womenReady, women.length, '♀ Women', 'bar-w')}
      ${genderBar(menReady, men.length, '♂ Men', 'bar-m')}
      ${slotGrid(members)}
      <div class="counts">
    ...
    <div class="card rules-card">
      ...
      <div class="rules-list">
        <div class="rule-row"><span class="rule-ico">1</span><span>Ten seats total — the circle fills as people join and finish the quiz. You can see live who's in and who's still answering.</span></div>
        <div class="rule-row"><span class="rule-ico">2</span><span>The moment all 10 seats are ready, matching locks automatically — no button, no host, nothing for anyone to press.</span></div>
        ...
        <div class="rule-row"><span class="rule-ico">↻</span><span>The instant that 2-minute window closes, the whole circle resets and 10 fresh seats open for the next round — automatically, for everyone.</span></div>
      </div>
    </div>
