...
async function aiWhyMatch(p, w, m) {
  if (!ANTHROPIC_API_KEY && !GEMINI_API_KEY) return null;
  try {
    let prompt = `Person A (${w.gender}): sun sign ${w.sunSign} (${p.ew}), element self-description ${w.vibeElement || 'n/a'}, relationship goal ${w.goal}, diet ${w.diet}.\n`;
    if (w.aiFollowupQuestion) prompt += `They were asked: "${w.aiFollowupQuestion}" and answered: "${w.aiFollowupAnswer}".\n`;
    prompt += `Person B (${m.gender}): sun sign ${m.sunSign} (${p.em}), element self-description ${m.vibeElement || 'n/a'}, relationship goal ${m.goal}, diet ${m.diet}.\n`;
    if (m.aiFollowupQuestion) prompt += `They were asked: "${m.aiFollowupQuestion}" and answered: "${m.aiFollowupAnswer}".\n`;
    prompt += `Compatibility breakdown: behavior fit ${p.behavior}%, shared habits ${p.habits}%, sun-sign fit ${p.sun}%, overall ${p.total}%.\nWrite the note now.`;
