...
function defaultState() {
  return { round: 1, members: [], locked: false, lockedAt: null, pairs: [], unmatchedIds: [], chats: {} };
}
function loadData() {
  try {
...
function scorePair(w, m) {
  ...
  let matches = 0;
  ['smoking', 'drinking', 'diet', 'goal'].forEach(f => { if (w[f] === m[f]) matches++; });
  const habits = (matches / 4) * 100;
...
app.post('/api/join', (req, res) => {
  ...
  const member = {
    ...
    smoking: null, drinking: null, diet: null, goal: null, idealPartner: '',
    joinedAt: Date.now()
  };
